    ...
    public static string[] ForPrint(Texts text)
    {
        ...
        switch (text)
        {
            ...
            case Texts.Description:
                str = ["Приветствую, пользователь!", "", "Данное приложение является мини-менеджером проектов.", "Задачи можно предоставить в виде CSV, ", "JSON файла или текстовым файлом.","", "Данные о задачах стоит предоставить так:", "[ID задачи] [Статус] [Приоритет] [Описание задачи]","", "Список статусов:", "• TODO","• IN_PROGRESS ","• DONE", "","Список приоритетов: ", "• Низкий" , "• Средний ", "• Высокий ", "", "Нажми любую кнопку для продолжения",""];
                return str;
            case Texts.ChoosePoint:
                str  = ["Меню ", "Выберите пункт меню", " ", "1. Ввести данные через файл", "2. Просмотреть все задачи", "3. Добавить новую задачу ", "4. Изменить статус задачи", "5. Удалить задачу по ID","6. Управление зависимостями", "7. Установить дедлайн для задачи ","8. Изменить процент выполнения " ,"9.Работа с проектами ", "F. Выйти из программы", ""];
                return str;
            ...
            case Texts.ChooseDependAct:
                str = ["Выберите пункт меню:", "","1. Добавить зависимость ", "2. Удалить зависимость", "3. Просмотреть зависимости",""];
                return str;
            case Texts.ChooseDeadLineAct:
                str = ["Выберите пункт меню:","","1. Добавить дедлайн ", "2. Удалить дедлайн"," "];
                return str;
            case Texts.WorkWithProjects:
                str = ["Выберите пункт меню: "," ","1. Создать новый проект", "2. Изменить название проекта ","3. Перенести задачу в другой проект","4. Удалить проект","5. Вывести статистку по проектам ",""];
                return str;
        }
        return null;
    }
